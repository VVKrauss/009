      ...
      {viewMode === 'list' ? (
        <div className="space-y-4">
          {filteredEvents.map(event => (
            <div key={event.id} className="card hover:shadow-md transition-shadow flex flex-col md:flex-row rounded-lg overflow-hidden">
              <div 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredEvents.map(event => (
            <Link 
              key={event.id} 
              to={`/events/${event.id}`}
              className="card group overflow-hidden hover:shadow-md transition-shadow rounded-lg"
            >
